...
public class JDBCExample1 {
	...
	public static void main(String[] args) {
		 ...
		 * - DB서버주소, 포트, DB이름, 사용자명, 비밀번호 등
		 * - Connection을 구현한 객체를 통해서 SQL을 전달하고 결과를 반환 받을 수 있다
		 * 
		 * 2) Statement
		 * - SQL을 DB에 전달
		 * - DB에 SQL 수행 결과를 반환 받는 역할
		 * 
		 * 3) ResultSet
		 * - SELECT 조회 결과를 저장하고 다루는 객체
		 * - 다룰 때 CURSOR(가리키는 객체)를 이용해 1행씩 접근!!
		 * 
		 * 4) DriverManager 클래스
		...
		try {
			 ...
			 * 	-> 작성 시 : "SQL 명렁어가 올바르게 종료되지 않았습니다" 예외 발생
			 *  왜? JDBC는 한 번에 SQL을 1개씩만 실행
			 *  => DB에 SQL 전달 시 자동으로 끝에 세미콜론(;) 붙여줌 
			 */
			...
			// ojbc10.jar에서 제공하는 OracleDriver 클래스가 없을 경우 발생하는 예외
			e.printStackTrace();
